...
namespace P05PasekLadowania
{
    public class ZawodnicyRepository
    {
        ...
        public HistoriaTemperatury[] PodajTemperaturyZAkutalnegoMiesiaca()
        {
            ModelBazyDanychDataContext db = new ModelBazyDanychDataContext();
            return db.HistoriaTemperatury.Where(x => x.dataSprawdzenia.Value.Month == DateTime.Now.Month).ToArray();
        }
    }
}